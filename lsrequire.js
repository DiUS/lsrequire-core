...
function pathFor(requireString, currentBrowserifyingFile) {
  ...
  if(matchedFiles.length > 1) {
    throw ("Error: Found multiple matches for: " + requireString + ": " + matchedFiles.join(","));
  }

  var matchedRequireFile = matchedFiles[0];

  if(isProdSource(currentBrowserifyingFile) && isTestSource(matchedRequireFile)) {
    throw ("Error: Production code cannot require test code: " + currentBrowserifyingFile + ", " + matchedRequireFile);
  }
...
global.lsrequire = function (requireFile) {
  if(requireFile.indexOf("$") === 0) {
    return require(pathFor(requireFile, __filename));
  } else {
